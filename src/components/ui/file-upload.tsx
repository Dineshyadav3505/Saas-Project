          ...
          <div className="relative w-full mt-10 max-w-xl mx-auto">
            ...
            {!files.length && (
              <motion.div
                ...
                className={cn(
                  "relative group-hover/file:shadow-2xl z-40 bg-zinc-500 flex items-center justify-center h-32 mt-4 w-full max-w-[8rem] mx-auto rounded-md",
      ...
      {Array.from({ length: rows }).map((_, row) =>
        Array.from({ length: columns }).map((_, col) => {
          ...
          return (
            <div
              key={`${col}-${row}`}
              className={`w-- h-10 flex flex-shrink-0 rounded-[2px] ${
                index % 2 === 0
                  ? "bg-black"
                  : "bg-black shadow-[0px_0px_1px_3px_rgba(255,255,255,1)_inset] dark:shadow-[0px_0px_1px_3px_rgba(0,0,0,1)_inset]"
              }`}
            />
          );
        })
